...
!-----------------------------------------------------------------------
     ...
     $     'Runtime paramere section for penalty module')
      ...
      ! register parameters
      call rprm_rp_reg(pen_nline_id,pen_sec_id,'NLINE',
     ...
     $     'Number of Fourier modes',rpar_int,0,0.0,.false.,' ')
         call rprm_rp_reg(pen_tdt_id(il),pen_sec_id,'TDT'//str,
...
!-----------------------------------------------------------------------
     ...
     $        'module ['//trim(pen_name)//'] already initiaised.')
      ...
      call rprm_rp_get(itmp,rtmp,ltmp,ctmp,pen_nline_id,rpar_int)
      pen_nline = itmp
     ...
     $        rpar_int)
         pen_nmode(il) = itmp
         call rprm_rp_get(itmp,rtmp,ltmp,ctmp,pen_tdt_id(il),
     $        rpar_real)
         pen_tdt(il) = rtmp
      enddo
     ...
     $           pen_spos(jl,il))**2
      ...
      do il=1,pen_nline
         pen_seed(il) = -32*il
      enddo
      pen_ntdt = 1 - pen_nset_max
      pen_ntdt_old = pen_ntdt
...
!-----------------------------------------------------------------------
      ...
      do il= 1, pen_nline
         ...
         ffn = pen_ftrp(ipos,il)*pen_fsmth(ix,iy,iz,iel,il)
...
!-----------------------------------------------------------------------
      ...
      ! time dependent part
      do il = 1, pen_nline
         itmp = int(time/pen_tdt(il))
         call bcast(itmp,ISIZE) ! just for safety
         do kl= pen_ntdt+1, itmp
...
!! @return      ran
      ...
      ! local variables
      integer iff(pen_nline_max), iy(pen_nline_max)
      integer ir(97,pen_nline_max)
      integer m,ia,ic,j
      real rm
      parameter (m=714025,ia=1366,ic=150889,rm=1./m)
      save iff,ir,iy
      data iff /pen_nline_max*0/
!-----------------------------------------------------------------------
      ! initialise
...
!-----------------------------------------------------------------------
     ...
     $                    sin(theta+pen_rphs(ll,jl,il))
         ...
         if (pen_ntdt.ne.pen_ntdt_old) then
#ifdef PENALTY_PR_RST
     ...
     $                    sin(theta+pen_rphs(kl,2,il))
      ...
      if (pen_tiamp.gt.0.0) then
         do il= 1, pen_nline
           call copy(pen_ftrp(1,il),pen_frcs(1,1,il),pen_npoint(il))
         enddo
      else
         do il= 1, pen_nline
            call rzero(pen_ftrp(1,il),pen_npoint(il))
         enddo
      endif
      ! interpolation in time
      do il = 1, pen_nline
         theta0= time/pen_tdt(il)-real(pen_ntdt)
         if (theta0.gt.0.0) then
            theta0=theta0*theta0*(3.0-2.0*theta0)
            !theta0=theta0*theta0*theta0*(10.0+(6.0*theta0-15.0)*theta0)
            do jl= 1, pen_npoint(il)
               pen_ftrp(jl,il) = pen_ftrp(jl,il) +
     ...
     $              theta0*pen_frcs(jl,3,il))
      ...
      do il=1,pen_npoint(1)
         write(iunit,*) il,pen_prj(il,1),pen_ftrp(il,1),
     $        pen_frcs(il,1:4,1)
      enddo
