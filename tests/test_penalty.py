...
@pytest.mark.parametrize("nb_regions", [2])
def test_penalty_regions(nb_regions):
    ...
    check_penalty_debug_output(sim, regions=tuple(range(1, nb_regions + 1)))
...
def check_penalty_debug_output(sim, regions):
    ...
    threshold = 1e-14
    y_coord[np.greater(threshold, y_coord)] = threshold

    abl_pen_k = y_coord * np.log(y_coord / z0)
    ...
    def mask_indices(region_idx):
        ...
        y_start = getattr(pen, f"sposy{idx}") - getattr(pen, f"smthy{idx}")
        y_end = getattr(pen, f"eposy{idx}") + getattr(pen, f"smthy{idx}")
        mask = (y_start <= ds1.y) & (ds1.y <= y_end)
        return mask
